<!-- https://github.com/dreamicalwestswim/vue3-cropper -->
<template>
  <teleport to="body">
            ...
            <img
              ...
              :style="{
                transform: `translate3d(${
                  originalImage.left - cropBox.left
                }px,${originalImage.top - cropBox.top}px,0) rotateZ(${
                  originalImage.rotate
                }deg) scale(${originalImage.scale})`,
              }"
            />
          </div>
...
let touchStartInfo = {
  ...
  touches: Array<any> | TouchList;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  mouseInfo: { [index: string]: any };
};
...
const loadImg = () => {
  ...
  // 获取图片的原始二进制数据字节数组
  getImageArrayBuffer(props.imagePath).then((arrayBuffer: ArrayBuffer) => {
    ...
    img.onload = (e: Event) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let { width, height } = e.target as any;
      const { maxImgSize } = props;
      // 方向等于1是正常的，尺寸未超出最大尺寸不需要处理
      if (
        orientation === 1 &&
        (!maxImgSize || (width <= maxImgSize && height <= maxImgSize))
      ) {
...
const initialState = () => {
  ...
  let ratio = props.ratio == 0 ? 1 : props.ratio;
  state.cropBox.width = props.cropSize * ratio;
  state.cropBox.height = props.cropSize;
  state.cropBox.left = (stage.value.clientWidth - props.cropSize * ratio) / 2;
 ...
 * 触摸开始
 */
const onTouchStart = (res: TouchEvent | MouseEvent) => {
  res.preventDefault();
  // 获取到触摸的目标名
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  dragTarget = (res as any).target.dataset.name;
  // 记录开始触摸的原图位置
  ...
  // 记录开始的触摸手势信息
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  if ((res as any).touches) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    touchStartInfo.touches = (res as any).touches;
    // 多指触碰屏幕开启手势缩放
    ...
    // 鼠标点击
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (touchStartInfo.mouseInfo as any).clientX = (res as any).clientX;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (touchStartInfo.mouseInfo as any).clientY = (res as any).clientY;
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const touchScale = (res: TouchEvent) => {
 ...
 * 触摸移动
 */
const onTouchMove = (res: MouseEvent | TouchEvent) => {
  // 缩放图片防止浏览器放大缩小
  res.preventDefault();
  // 双指操作改变图片比例
  if (res instanceof TouchEvent && res.touches && res.touches.length === 2) {
    touchScale(res);
    return;
  }
  // 正在缩放的情况下不触发单指操作
  ...
  if (res instanceof TouchEvent) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    disX = res.touches[0].clientX - (touches[0] as any).clientX;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    disY = res.touches[0].clientY - (touches[0] as any).clientY;
  } else {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    disX = res.clientX - (mouseInfo as any).clientX;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    disY = res.clientY - (mouseInfo as any).clientY;
  }
  ...
  if (!dragTarget) {
    ...
    const justifyRatio = (
      independence: "width" | "height" | "both" /* 自变量 */,
      movingPoint: "top" | "left" | "both" | "none" /* 动点 */
    ) => {
      if (ratio != 0) {
        switch (independence) {
          case "both":
    ...
    switch (dragTarget) {
      case "dot-tl":
        ...
        justifyRatio("both", "both");
        break;
      case "dot-tc":
        // console.log("上中");
        height = cropHeight - disY;
        justifyRatio("height", "top");
        break;
      case "dot-tr":
        // console.log("右上");
        width = cropWidth + disX;
        height = cropHeight - disY;
        justifyRatio("both", "top");
        break;
      case "dot-rc":
        // console.log("右中");
        width = cropWidth + disX;
        justifyRatio("width", "none");
        break;
      case "dot-rb":
        // console.log("右下");
        width = cropWidth + disX;
        height = cropHeight + disY;
        justifyRatio("both", "none");
        break;
      case "dot-bc":
        // console.log("下中");
        height = cropHeight + disY;
        justifyRatio("height", "none");
        break;
      case "dot-bl":
        // console.log("左下");
        width = cropWidth - disX;
        height = cropHeight + disY;
        justifyRatio("both", "left");
        break;
      case "dot-lc":
        // console.log("左中");
        width = cropWidth - disX;
        justifyRatio("width", "left");
        break;
      case "mask":
        // console.log("拖拽");
        left = cropLeft + disX;
...
const onCrop = () => {
  ...
  let { width: w, height: h } = state.outputSize;
  // 图片的当前信息
...
const { img, originalImage, cropBox, outputSize, loadComplete } = toRefs(state);
</script>
